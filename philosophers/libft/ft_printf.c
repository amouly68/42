...
/*   Updated: 2022/12/18 10:39:06 by amouly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse(const char c, va_list ptr, int *count)
{
	...
	verif = *count;
	if (c == 'c')
		*count += (ft_putchar_fd_printf(va_arg(ptr, int), 1));
	if (c == '%')
		*count += (write(1, "%", 1));
	if (c == 's')
		*count += (ft_putstr_fd_printf(va_arg(ptr, char *), 1));
	if (c == 'p')
		*count += (ft_putpointer(va_arg(ptr, unsigned long long int)));
	if (c == 'd' || c == 'i')
		*count += (ft_putnbr_fd_printf(va_arg(ptr, int), 1));
	if (c == 'u')
...
int	ft_printf(const char *str, ...)
{
	...
	while (*str)
	{
		if (*str != '%')
		{
			if (ft_putchar_fd_printf(*str, 1) == -1)
				return (-1);
			count++;
		}
		else if (*str == '%')
