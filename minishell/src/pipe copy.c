...
/*   Updated: 2023/03/19 15:22:08 by amouly           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    init_fd(t_pipe *pipe_info, t_command *list, int i)
{
    int fd_temp;
    (void) list;
    //(void)  i;
    
    fd_temp = open(".temp_file", O_RDWR, O_CREAT);
    pipe_info->fd_reading = STDIN_FILENO ;
    pipe_info->fd_writing = STDOUT_FILENO;
    if (i > 0)
        pipe_info->fd_reading = fd_temp;
    if (i < pipe_info->nbr_of_pipes - 1)
        pipe_info->fd_writing = fd_temp;
    pipe_info->fd_temp = fd_temp;
}
